<?php
function eager_Jc8tYPrySUxWuirA_init() {
  eager_Jc8tYPrySUxWuirA_get_user_id();
  eager_Jc8tYPrySUxWuirA_get_site_id();
}

function eager_Jc8tYPrySUxWuirA_create_site(){
  ...
  $resp = wp_remote_post($url, array(
    'headers' => array(
      ...
      'Authorization' => eager_Jc8tYPrySUxWuirA_get_token(),
    ),
...
function eager_Jc8tYPrySUxWuirA_get_installs(){
...
function eager_Jc8tYPrySUxWuirA_uninstall_app($appId){
  $installs = eager_Jc8tYPrySUxWuirA_get_installs();

  foreach ($installs as $install) {
    if ($install['appId'] == $appId && $install['active'] && !$install['pending']){
      return eager_Jc8tYPrySUxWuirA_delete_install($install['id']);
    }
  }
}

function eager_Jc8tYPrySUxWuirA_delete_install($installId){
...
function eager_Jc8tYPrySUxWuirA_create_user(){
...
function eager_Jc8tYPrySUxWuirA_get_site_id(){
  $siteId = get_option('eager_site_id');

  if (!$siteId){
    $siteId = eager_Jc8tYPrySUxWuirA_create_site();
  }

  return $siteId;
}

function eager_Jc8tYPrySUxWuirA_get_user_id(){
  $userId = get_option('eager_user_id');

  if (!$userId){
    $userId = eager_Jc8tYPrySUxWuirA_create_user();
  }

  return $userId;
}

function eager_Jc8tYPrySUxWuirA_get_token(){
  eager_Jc8tYPrySUxWuirA_get_user_id();
  
  return get_option('eager_access_token');
}

function eager_Jc8tYPrySUxWuirA_transfer_site($destUserId){
  $url = "https://api.eager.io/site/" . eager_Jc8tYPrySUxWuirA_get_site_id() . "/transfer";
  $body = array(
...
function eager_Jc8tYPrySUxWuirA_deactivate_plugin($appId){
